...
impl WeekTemperatures {
    ...
    pub fn get_temperature(&self, day: Weekday) -> Option<i32> {
        let index = weekday_2_index(&day);
        // self.temperatures[index]
        self.temperatures.get(index).and_then(|value| *value)
    }

    pub fn set_temperature(&mut self, day: Weekday, temperature: i32) {
        let index = weekday_2_index(&day);
        if let Some(value) = self.temperatures.get_mut(index) {
            *value = Some(temperature);
        }
    }
}

fn weekday_2_index(day: &Weekday) -> usize {
    match day {
        Weekday::Monday => 0,
        Weekday::Tuesday => 1,
        Weekday::Wednesday => 2,
